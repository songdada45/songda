...
# ─── 配置 ────────────────────────────────────────────
INDICES = {
    "growth_cyb":  {"name": "科创50",  "code": "000688", "market": "sh"},
...
CACHE_DIR = Path.cwd() / ".kechuang50_cache"
...
BASKET_INDEX = {"code": "000688", "market": "sh"}   # 科创50 (基准/调仓来源)

# 13 只曾进入 TOP3 的个股 (覆盖 2014-06 ~ 2026-06 全部 21 期)
DELISTED_BASKET = set()   # 已退市篮子股(用qfq + 末日后置0); 科创50 TOP3 当前无退市
STOCK_CODES = ["688111", "688012", "688009", "688981", "688599", "688256", "688041"]

# ─── TOP3 篮子换仓时间口径 (2026-08-13 按用户要求订正) ───
# 中证样本股调整方案通常在实施前约2周(约10个交易日)公布 (招募说明书明载:
...
ANNOUNCE_GAP = 10   # 生效日前约10个交易日 = 中证「发布日」(权重清单收盘后才公布)
...
_SCHEDULE_EFFECTIVE = {
    # —— 科创50 TOP3 历史重建 (2020-07-23 指数发布 ~ 2026-06-15) ——
    # 置信度: HIGH=官方权重公告/调整公告明确; MED/LOW=市值推断(免费源无历史权重)
    "2020-07-23": ["688111", "688012", "688009"],   # 发布期 MED: 金山办公/中微/中国通号
    "2020-12-14": ["688981", "688111", "688012"],   # MED: 中芯国际调入成#1
    "2021-06-15": ["688981", "688111", "688599"],   # LOW-MED: 天合成#3(推断)
    "2022-03-14": ["688981", "688599", "688111"],   # LOW-MED: 季度调样(推断)
    "2022-09-09": ["688981", "688599", "688111"],   # LOW-MED
    "2023-03-10": ["688981", "688599", "688111"],   # HIGH: 公告明确前五大
    "2023-06-12": ["688981", "688111", "688012"],   # HIGH: 中微回升#3
    "2023-12-11": ["688981", "688111", "688012"],   # HIGH
    "2024-03-11": ["688981", "688041", "688111"],   # HIGH: 海光升#2
    "2024-06-17": ["688981", "688041", "688111"],   # MED
    "2024-12-16": ["688981", "688041", "688256"],   # HIGH: 寒武纪升#3
    "2025-06-16": ["688256", "688981", "688041"],   # MED: 寒武纪升#1(外推)
    "2025-12-15": ["688256", "688981", "688041"],   # MED-HIGH
    "2026-06-15": ["688256", "688012", "688041"],   # HIGH: 与中证2026-07-31快照一致(当前期)
}

# 14 期 TOP3; 执行日(发布日+EXEC_DELAY) → TOP3 ; 驱动 get_bucket_codes / simulate / 回测
...
# 个股简称 (报告文字用)
STOCK_NAMES = {"688111": "金山办公", "688012": "中微公司", "688009": "中国通号",
               "688981": "中芯国际", "688599": "天合光能", "688256": "寒武纪",
               "688041": "海光信息"}

# 中证科创50 调仓规则: 每年3/6/9/12月第二个星期五的下一交易日生效 (季度审议+快速纳入)
CSINDEX_CLOSEWEIGHT_URL = "https://oss-ch.csindex.com.cn/static/html/csindex/public/uploads/file/autofile/closeweight/000688closeweight.xls"
...
def load_long_csv_data() -> dict[str, list[dict]] | None:
    """从博主15年CSV数据加载更长历史 (用于年度涨幅表 / 只做成长曲线扩展时间轴)
    
    CSV 来源: ZF1Huang/growth_dividend_rotation_research data_cache/
    数据范围:
      - 000688 (科创50): 2020-07-23 起 (~6.5年)
...
def fetch_kline_basket(market, code, start="2020-07-23", end="2026-12-31", adj="hfq"):
...
def _resolve_bucket(date_str: str, override: dict | None = None) -> list:
    """统一解析某日应持 TOP3: 中证已变更且属当前期时, 用 override 实时名单;
...
def prepare_backtest_data() -> dict | None:
    """一次性加载并合并长数据 + 腾讯API最新日, 计算科创50/红利策略 NAV/信号/逐年收益。

    供 compose_all_charts 共用,
    消除原设计中科创对 NAV 被算两遍、API 被调 6 次的冗余。
    ...
    cyb_map, cyb_open = _merge_price_maps(csv_data["000688"] if csv_data else None, api_cyb)
...
def compute_combined_strategy(top3_override=None) -> dict | None:
    ...
    try:
        ...
        for code in STOCK_CODES:
            ...
            o, c = align_to_master_basket(recs, master_dates, zero_after_last=(code in DELISTED_BASKET))
            stock_open[code] = o
            stock_close[code] = c
...
# ─── 资讯联动: 中证科创50 实时 TOP3 + 下次调仓 ───────────

def fetch_kc50_top3() -> list | None:
    """联网抓中证科创50 前3权重股(含权重%)。失败返回 None。xlrd 解析官方 closeweight.xls。"""
    import io as _io, urllib.request as _urllib, ssl as _ssl, xlrd as _xlrd
    try:
        req = _urllib.Request(
            CSINDEX_CLOSEWEIGHT_URL,
            headers={"User-Agent": "Mozilla/5.0",
                     "Referer": "https://www.csindex.com.cn/"})
        ctx = _ssl.create_default_context()
        ctx.check_hostname = False
        ctx.verify_mode = _ssl.CERT_NONE
        data = _urllib.urlopen(req, timeout=30, context=ctx).read()
        wb = _xlrd.open_workbook(file_contents=data)
        ws = wb.sheet_by_index(0)
        recs = []
        for r in range(1, ws.nrows):
            try:
                d = str(int(ws.cell_value(r, 0)))
                code = str(int(ws.cell_value(r, 4))).zfill(6)
                name = ws.cell_value(r, 5)
                w = float(ws.cell_value(r, 9))
                recs.append({"date": d, "code": code, "name": name, "weight": w})
            except (ValueError, TypeError):
                continue
        if not recs:
            return None
        # 取「最新快照日」的前3权重股 —— 防御: 若返回多日历史样本,
...
def get_current_basket_info() -> dict:
    ...
    if today.month in (5, 11) and today.day >= 20:
        key_dates.append("关注中证调样公告（通常生效前约2周发布）")
    ...
    # 以「实时(中证)或硬编码」当前期为准计算新增 (中证变更时高亮更准确)
...
def verify_data_freshness(basket_info: dict | None = None) -> dict:
    """数据准确性校验层: 每次运行实测 TOP3(中证实时) / 指数价 / 个股价 新鲜度与一致性。

    返回结构化结果供报告头/飞书展示, overall ∈ {OK, WARN, CRITICAL}:
      - TOP3: 中证实时抓取 ↔ 硬编码 SCHEDULE 交叉核对。
          · VERIFIED        中证与名单一致
          · UNVERIFIED      中证抓取失败(无法联网验证, 名单为硬编码兜底)
          · MISMATCH_CRITICAL 中证已变更但名单未同步 → 影响收益
      - 指数/个股价: 比对缓存最新日与「最近交易日」, 滞后→STALE。
    CRITICAL 且属当前期时, 主流程改用中证实时 TOP3 计算当前开放期(绝不静默用旧名单)。
    """
    today = date.today()
    ...
    if live is None:
        ...
        top3_detail = "中证官网抓取失败 → TOP3 沿用硬编码名单(未经联网验证)"
        msgs.append("⚠ TOP3 未经联网验证: 中证官网抓取失败, 当前名单为硬编码兜底")
        live_le_cur = False
    elif match:
        top3_state = "VERIFIED"
        top3_detail = f"中证官网 {live_date} 与硬编码名单一致 ✓"
        live_le_cur = True
    else:
        # 中证快照晚于当前执行期 → 属「未来期已公布」(不应误用); 否则属「当前期已变更」
        live_le_cur = (live_date <= cur_exec)
        if live_le_cur:
            top3_state = "MISMATCH_CRITICAL"
            top3_detail = (f"⚠ 中证 {live_date} 已变更为 {live} 但硬编码名单仍为 {hard} "
                           f"→ 当前开放期改按中证实时TOP3计算")
            msgs.append(f"⚠【关键】中证TOP3已更新({live_date})但名单未同步, 已改用实时TOP3算当前期")
        else:
            top3_state = "MISMATCH_CRITICAL"
            top3_detail = (f"⚠ 中证 {live_date} 公布未来期TOP3 {live}, 当前名单 {hard} 未更新 "
                           f"→ 请同步SCHEDULE; 当前收益仍按现名单(避免误用未来期)")
            msgs.append(f"⚠【关键】中证已公布未来期TOP3({live_date}), 请更新SCHEDULE名单")

    # ── 2. 指数价 (科创50 / 中证红利) ──
    idx_stale = []
    for code in ("000688", "000922"):
    ...
    # ── 3. 个股价 (当前 TOP3 个股; 000688 科创50指数作为信号源已在第2步校验) ──
    check_codes = list(dict.fromkeys(live or hard))
    basket_stale = []
    for code in check_codes:
        cf = BASKET_CACHE_DIR / f"sh{code}_hfq.json"
        if not cf.exists():
            cf = BASKET_CACHE_DIR / f"sh{code}_qfq.json"
...
def _compute_holding_periods(combined: dict, top3_override=None) -> list[dict]:
    ...
    返回每行: {buy, sell, top3, ret, days, status}
      - buy:  成长期首日出 (信号次日开盘买入)
      - sell: 下一交易日(红利期首日, 开盘卖出); 若仍持有则为 None
      - top3: 该期科创50 TOP3 代码 (get_bucket_codes)
      - ret:  该周期策略 NAV 区间收益
      - days: 持有交易日数
      - status: 持有中 / 已清仓
    红利期(positions==0 / None) 跳过 —— 即"空仓等待"阶段。
    """
    dates = combined.get("dates", [])
    positions = combined.get("positions", [])
    nav = combined.get("nav", [])
    if not (dates and positions and nav):
        return []
    if not (len(dates) == len(positions) == len(nav)):
        return []
    periods = []
    n = len(dates)
    i = 0
    while i < n:
        if positions[i] == 1:
            ...
            periods.append({"buy": buy, "sell": sell, "top3": top3,
                            "ret": ret, "days": days, "status": status})
            i = j
        else:
            i += 1
    return periods
...
def compose_all_charts(chart_files: list[tuple[str, str]], save_path: str,
    ...
    def _draw_annual_table(draw, x: int, y: int, width: int,
        ...
        HEADER_ROW_H = 84
        ROW_H = 76
        ty = y + 64 + len(_sub_lines) * 52 + 12  # 表头起始 y (含换行副标题)
        ...
        for i, yr in enumerate(all_years):
            ...
            draw.text((x + 20, ry + 24), yr_label, fill=CAPTION_COLOR, font=font_small)
            ...
            if yr in comb_hold:
                ...
                try:
                    ...
                    pass
                draw.text((x_hold + 16, ry + 24), _htxt, fill=CAPTION_COLOR, font=_hf)
            else:
                draw.text((x_hold + 16, ry + 24), "—", fill=SUBTLE_COLOR, font=font_small)
            ry += ROW_H
    ...
    def _draw_holding_periods(draw, x: int, y: int, width: int, periods: list[dict],
        ...
        """
        # 构建科创50 date→close 映射(用于计算该时段指数涨跌幅对比)
        if cyb_prices is None:
            try:
                import json as _json
                _cp = CACHE_DIR / "000688.json"
                if _cp.exists():
        ...
        w_ret = width * 0.13
        w_cyb = width * 0.13        # 科创50涨幅（与区间收益等宽）
        ...
        HEADER_ROW_H = 84
        ROW_H = 76
        ty = y + 64 + len(wrap_text(sub, font_small, width - 8)) * 44 + 12

        draw.rectangle([(x, ty), (x + width, ty + HEADER_ROW_H)], fill=(22, 27, 34))
        draw.line([(x, ty + HEADER_ROW_H), (x + width, ty + HEADER_ROW_H)],
                  fill=SEP_COLOR, width=2)
        _hx = ty + 24
        draw.text((x_buy + 16, _hx), "买入日期", fill=CAPTION_COLOR, font=font_small)
        ...
        draw.text((x_cyb + 16, _hx), "科创50", fill=CAPTION_COLOR, font=font_small)
        ...
        for i, p in enumerate(periods):
            ...
            draw.rectangle([(x, ry), (x + width, ry + ROW_H)], fill=row_bg)
            ry24 = ry + 24
            draw.text((x_buy + 16, ry24), p["buy"], fill=CAPTION_COLOR, font=font_small)
            ...
            _avail = w_top3 - 32
            try:
                if draw.textlength(names, font=font_small) > _avail:
                    _hf = _load_font(30)
            except Exception:
                pass
    ...
    for i, (img, (title_key, _path)) in enumerate(zip(images, chart_files)):
        # 仅科创策略, results 只含 growth_cyb (r_idx 恒为 0)
        r_idx = 0
    ...
    if annual_returns:
        ...
        if _ally:
            ...
            annual_height = 64 + _sub_n * 52 + 12 + 84 + _row_count * 76 + 30
    ...
    if combined:
        ...
        _sub_text = ("红利期=空仓等待(持币, 收益=0); 成长期=满仓持有当期TOP3等权。"
                     "买入/卖出日期均为信号次日开盘执行。")
        _sub_n = len(wrap_text(_sub_text, font_small, _w - 8))
...
def main():
    ...
    # 资讯联动: 当前篮子 TOP3 + 下次调仓 (联网抓取中证官网, 失败兜底)
    try:
    ...
    # 数据准确性校验层: 实测 TOP3(中证实时)/指数价/个股价 新鲜度与一致性
    # 早期跑一次: 仅用 TOP3 部分(中证联网, 不依赖价格缓存)决定开放期是否改用实时名单;
    ...
    if fres_early and fres_early["top3"]["state"] == "MISMATCH_CRITICAL" \
        ...
        # 中证已变更且属当前期 → 当前开放期改用中证实时TOP3(避免用旧名单算收益)
    ...
    if freshness:
        ...
        _line = (f"\n\n【数据准确性校验】总评={_ov} | TOP3:{_t3['state']}"
                 f"(中证{_t3.get('live_date','?')}) | 指数:{freshness['index']['state']}"
